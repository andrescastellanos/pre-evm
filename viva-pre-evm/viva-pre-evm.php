...
// ═══════════════════════════════════════════════════════════════
// LOGO BASE64 — reutilizado en shortcode JS y página de resultado
// ═══════════════════════════════════════════════════════════════
function viva_preevm_logo_b64() {
...
function viva_render_user_result_page( $r ) {
    ...
    <div class="vpu">
      ...
      <!-- ── Botón descarga PDF ── -->
      <div style="text-align:center;margin-top:28px;padding-top:20px;border-top:1px solid rgba(255,255,255,.07)">
        <button
          id="vpu-pdf-btn"
          onclick="vpuDownloadPDF()"
          style="display:inline-flex;align-items:center;gap:10px;background:#E8600A;color:#fff;font-weight:700;font-size:15px;padding:14px 32px;border-radius:10px;border:none;cursor:pointer;font-family:'Outfit',sans-serif;letter-spacing:.3px"
        >
        ...
        <p style="font-size:12px;color:rgba(255,255,255,.38);margin:10px 0 0">Informe Preliminar de Evaluaci&oacute;n de Viabilidad Migratoria</p>
      </div>
    ...
    <script>
    (function(){
      var vpuData = <?php echo wp_json_encode([
        'nom'       => $r['nom']       ?? '',
        'ape'       => $r['ape']       ?? '',
        'prof'      => $r['prof']      ?? '',
        'pais'      => $r['pais']      ?? '',
        ...
        'viability' => $r['viability'] ?? 'no-apto',
        'alcance'   => $r['alcance']   ?? '',
        'academico' => $r['academico'] ?? '',
        'laboral'   => $r['laboral']   ?? '',
        'anzsco'    => is_array( $r['anzsco']    ?? null ) ? $r['anzsco']    : [],
        'variables' => is_array( $r['variables'] ?? null ) ? $r['variables'] : [],
      ]); ?>;
      var LOGO_PDF_B64 = <?php echo json_encode( viva_preevm_logo_b64() ); ?>;

      window.vpuDownloadPDF = function(){
        if(typeof window.jspdf==='undefined'){
          var s=document.createElement('script');
          s.src='https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js';
          s.onload=function(){doGenerate();};
          document.head.appendChild(s);
        } else { doGenerate(); }
      };

      function doGenerate(){
        var btn=document.getElementById('vpu-pdf-btn');
        if(btn){btn.disabled=true;btn.textContent='Generando PDF...';}
        try{
          var jsPDF=window.jspdf.jsPDF;
          var doc=new jsPDF({unit:'mm',format:'a4'});
          var d=vpuData;
          var W=210,H=297,M=20,TW=170;
          var coR=232,coG=96,coB=10;
          var cy=33;

          var drawGradient=function(x,y0,w,h,r1,g1,b1,r2,g2,b2){
            var bands=24,bh=h/bands;
            for(var i=0;i<bands;i++){
              var t=i/bands;
              doc.setFillColor(Math.round(r1+(r2-r1)*t),Math.round(g1+(g2-g1)*t),Math.round(b1+(b2-b1)*t));
              doc.rect(x,y0+i*bh,w,bh+.5,'F');
            }
          };
          ...
          var newPage=function(){
            ...
            doc.setFillColor(coR,coG,coB);doc.rect(0,0,W,3,'F');
            try{doc.addImage('data:image/jpeg;base64,'+LOGO_PDF_B64,'JPEG',W-M-34,5,34,17);}
            catch(e){doc.setFont('helvetica','bold');doc.setFontSize(7);doc.setTextColor(coR,coG,coB);doc.text('VIVA AUSTRALIA',W-M,13,{align:'right'});}
            doc.setFillColor(232,236,242);doc.rect(M,27,TW,.3,'F');
            cy=33;
          };

          var addSection=function(num,title,body){
            var lines=doc.splitTextToSize(body||'',TW-9);
            var needH=17+lines.length*5.2+10;
            if(cy+needH>H-16){newPage();}
            doc.setFillColor(coR,coG,coB);doc.roundedRect(M,cy,8,8,1.5,1.5,'F');
            doc.setFont('helvetica','bold');doc.setFontSize(8.5);doc.setTextColor(255,255,255);
            doc.text(String(num),M+4,cy+5.8,{align:'center'});
            doc.setFont('helvetica','bold');doc.setFontSize(11);doc.setTextColor(10,22,42);
            doc.text(title,M+12,cy+6);
            cy+=13;
            doc.setFillColor(coR,coG,coB);doc.rect(M,cy,38,.8,'F');
            cy+=7;
            doc.setFont('helvetica','normal');doc.setFontSize(9.5);doc.setTextColor(55,70,88);
            lines.forEach(function(l){
              if(cy>H-18){newPage();}
              doc.text(l,M+4,cy);cy+=5.2;
            });
            cy+=8;
          };

          // PORTADA
          drawGradient(0,0,W,H,232,96,10,245,170,28);
          doc.setFillColor(245,148,20);doc.ellipse(178,228,52,52,'F');
          doc.setFillColor(238,130,12);doc.ellipse(183,262,42,42,'F');
          doc.setFillColor(228,112,8); doc.ellipse(166,254,36,36,'F');
          doc.setFillColor(255,242,210);doc.roundedRect(148,196,58,42,2,2,'F');
          doc.setFillColor(255,255,255);doc.roundedRect(152,200,50,36,1,1,'F');
          doc.setFillColor(205,212,222);
          doc.rect(157,206,30,1.2,'F');doc.rect(157,210,22,1.2,'F');
          doc.rect(157,214,27,1.2,'F');doc.rect(157,218,18,1.2,'F');
          doc.rect(157,222,24,1.2,'F');doc.rect(157,226,20,1.2,'F');
          try{doc.addImage('data:image/jpeg;base64,'+LOGO_PDF_B64,'JPEG',M,14,56,28);}
          catch(e){doc.setFont('helvetica','bold');doc.setFontSize(16);doc.setTextColor(255,255,255);doc.text('VIVA AUSTRALIA',M,28);}
          doc.setFillColor(255,255,255);doc.rect(M,66,5,72,'F');
          var tx=M+11;
          doc.setFont('helvetica','normal');doc.setFontSize(23);doc.setTextColor(255,255,255);
          doc.text('INFORME',tx,81);
          var infW=doc.getTextWidth('INFORME ');
          doc.setFont('helvetica','bold');doc.setFontSize(23);
          doc.text('PRELIMINAR',tx+infW,81);
          doc.setFont('helvetica','bold');doc.setFontSize(23);
          doc.text('DE EVALUACI\u00d3N DE',tx,95);
          doc.text('VIABILIDAD MIGRATORIA',tx,109);
          doc.setFont('helvetica','normal');doc.setFontSize(15);doc.setTextColor(255,232,165);
          doc.text('Para: '+(d.nom||'')+' '+(d.ape||''),tx,125);

          // ÍNDICE
          doc.addPage();
          doc.setFillColor(248,249,250);doc.rect(0,0,W,H,'F');
          try{doc.addImage('data:image/jpeg;base64,'+LOGO_PDF_B64,'JPEG',M,16,56,28);}
          catch(e){doc.setFont('helvetica','bold');doc.setFontSize(13);doc.setTextColor(coR,coG,coB);doc.text('VIVA AUSTRALIA',M,28);}
          doc.setFont('helvetica','bolditalic');doc.setFontSize(36);doc.setTextColor(coR,coG,coB);
          doc.text('\u00cdndice',M,76);
          doc.setFillColor(coR,coG,coB);doc.rect(M,80,48,2.5,'F');
          doc.setFont('helvetica','bold');doc.setFontSize(11);doc.setTextColor(10,22,42);
          doc.text('Programa de Migraci\u00f3n Calificada de Australia',M,95);
          doc.text('(Skilled Migration Programme)',M,103);
          var idxSecs=['Naturaleza y alcance del informe','An\u00e1lisis acad\u00e9mico detallado','An\u00e1lisis laboral desarrollado','Marco regulatorio y ocupacional','Variables de competitividad a revisar'];
          if(d.viability!=='no-apto'){
            idxSecs.push('Definici\u00f3n de la Evaluaci\u00f3n de Viabilidad Migratoria (EVM)');
            idxSecs.push('Honorarios y condici\u00f3n comercial');
          }
          doc.setFont('helvetica','normal');doc.setFontSize(11);doc.setTextColor(60,80,105);
          var iy=117;
          idxSecs.forEach(function(s,i){doc.text((i+1)+'.  '+s,M+8,iy);iy+=11;});
          doc.setFillColor(coR,coG,coB);doc.ellipse(168,255,40,40,'F');
          doc.setFillColor(245,162,30);doc.ellipse(168,255,33,33,'F');
          doc.setFillColor(235,148,18);doc.ellipse(168,255,24,24,'F');
          doc.setFont('helvetica','bold');doc.setFontSize(7.5);doc.setTextColor(255,255,255);
          doc.text('VIVA',168,252,{align:'center'});
          doc.text('AUSTRALIA',168,259,{align:'center'});

          // CONTENIDO
          newPage();
          doc.setFillColor(250,251,253);doc.roundedRect(M,cy,TW,30,3,3,'F');
          doc.setFillColor(coR,coG,coB);doc.rect(M,cy,3,30,'F');
          doc.setFont('helvetica','bold');doc.setFontSize(14);doc.setTextColor(10,22,42);
          doc.text((d.nom||'')+' '+(d.ape||''),M+8,cy+11);
          doc.setFont('helvetica','normal');doc.setFontSize(9);doc.setTextColor(90,108,130);
          doc.text((d.prof||'')+(d.pais?' \u00b7 '+d.pais:''),M+8,cy+19);
          doc.text('Edad: '+(d.edad||'\u2014')+'   \u00b7   Ingl\u00e9s: '+(d.eng||'\u2014'),M+8,cy+26);
          cy+=38;

          addSection(1,'Naturaleza y alcance del informe',d.alcance);
          addSection(2,'An\u00e1lisis acad\u00e9mico detallado',d.academico);
          addSection(3,'An\u00e1lisis laboral desarrollado',d.laboral);

          var anzscoBody='El sistema migratorio australiano utiliza la clasificaci\u00f3n ANZSCO para identificar ocupaciones calificadas. Para validar la experiencia se requiere un Skills Assessment emitido por autoridades competentes (CPA Australia, CA ANZ o IPA).';
          if((d.anzsco||[]).length>0){
            anzscoBody+='\n\nC\u00f3digos ANZSCO orientativos (no definitivos en esta fase):\n';
            (d.anzsco||[]).forEach(function(a){anzscoBody+=a.code+' \u2013 '+a.name+(a.note?' ('+a.note+')':'')+'\n';});
          }
          addSection(4,'Marco regulatorio y ocupacional',anzscoBody);

          var varBody=(d.variables||[]).map(function(v){return '\u2022 '+v.title+': '+v.desc;}).join('\n')||'\u2014';
          addSection(5,'Variables de competitividad a revisar',varBody);

          if(d.viability!=='no-apto'){
            addSection(6,'Definici\u00f3n de la Evaluaci\u00f3n de Viabilidad Migratoria (EVM)',
              'La EVM es un an\u00e1lisis t\u00e9cnico-jur\u00eddico profundo realizado por nuestro Agente Migratorio Registrado. En ella se define la estrategia legal exacta, se calcula el puntaje real, se identifican los riesgos normativos (incluyendo el an\u00e1lisis de antecedentes que puedan impactar futuras solicitudes) y se establece una hoja de ruta con tiempos y costos totales del proceso. Solo se recomienda cuando, como en tu caso, existe una base jur\u00eddica real para avanzar.');
            addSection(7,'Honorarios y condici\u00f3n comercial',
              'El valor est\u00e1ndar oficial de la Evaluaci\u00f3n de Viabilidad Migratoria (EVM) es de USD 270.\n\nSi decides iniciar tu proceso formal dentro de los 30 d\u00edas posteriores a la entrega de la EVM, el valor pagado se descuenta del total de honorarios profesionales.');
          }

          // Disclaimer
          if(cy+38>H-16){newPage();}
          doc.setFillColor(248,249,250);doc.roundedRect(M,cy,TW,38,2,2,'F');
          doc.setFillColor(coR,coG,coB);doc.rect(M,cy,2.5,38,'F');
          cy+=6;
          doc.setFont('helvetica','bold');doc.setFontSize(8.5);doc.setTextColor(50,65,85);
          doc.text('Validaci\u00f3n profesional:',M+6,cy);cy+=5.5;
          doc.setFont('helvetica','normal');doc.setFontSize(7.8);
          var dsc='Esta informaci\u00f3n ha sido revisada bajo los est\u00e1ndares de an\u00e1lisis de perfil utilizados por Viva Australia Internacional por nuestro Agente Migratorio Senior Registrado Frank Cross (MARN 0101111), certificado por la Migration Agents Registration Authority (MARA) de Australia. Este an\u00e1lisis tiene car\u00e1cter preliminar, informativo y orientativo, y no constituye asesor\u00eda migratoria formal ni recomendaci\u00f3n migratoria personalizada. La asesor\u00eda migratoria detallada, personalizada y jur\u00eddicamente aplicable solo puede proporcionarse dentro de un proceso formal de evaluaci\u00f3n profesional.';
          var dl=doc.splitTextToSize(dsc,TW-10);
          dl.forEach(function(l){if(cy<H-16){doc.text(l,M+6,cy);cy+=4.3;}});

          // Footer
          var totalP=doc.getNumberOfPages();
          for(var p=1;p<=totalP;p++){
            doc.setPage(p);
            doc.setFillColor(coR,coG,coB);doc.rect(0,H-9,W,9,'F');
            doc.setFont('helvetica','normal');doc.setFontSize(6.5);doc.setTextColor(255,255,255);
            doc.text('\u00a9 2026 Viva Australia Internacional \u00b7 MARA 0101111 \u00b7 Frank Cross, Senior Migration Agent \u00b7 Informe orientativo generado por IA',M,H-3);
            doc.text('P\u00e1g. '+p+'/'+totalP,W-M,H-3,{align:'right'});
          }

          doc.save('Informe-Pre-EVM_'+(d.nom||'')+'_'+(d.ape||'')+'.pdf');
        } catch(err){
          console.error('Error generando PDF:', err);
          alert('Error al generar el PDF. Por favor intenta de nuevo.');
        } finally {
          if(btn){btn.disabled=false;btn.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Descargar Informe PDF';}
        }
      }
    })();
    </script>
...
// ═══════════════════════════════════════════════════════════════
// HELPER — Escapa emojis Unicode en strings JS para evitar que
...
function viva_preevm_shortcode( $atts ) {
    ...
    // Logo fuera del HEREDOC para evitar crash JS por string de 80KB dentro del bloque
    echo '<script>window._viva_logo_b64=' . json_encode( viva_preevm_logo_b64() ) . ';</script>';

    // Debug solo para admins
    $debug_script = '';
    if ( current_user_can( 'administrator' ) ) {
        $debug_script = '<script>window.__VIVA_DEBUG=true;console.log("%c[Viva Pre-EVM] DEBUG MODE ACTIVO","color:#E8600A;font-weight:bold;font-size:14px");console.log("cfg:",window._viva_preevm_cfg);</script>';
    }
    echo $debug_script;
...
VPHTML;
    ...
    // Capturar el HTML generado (sin el <script>) antes de que WordPress
    // procese emojis — el JS se genera por separado para poder escaparlo
    $html_output = ob_get_clean();

    // Generar el bloque <script> en un buffer separado y escapar emojis
    // para evitar que wp_staticize_emoji corrompa los pares surrogados UTF-16
    ob_start();
    echo <<<VPJS
<script>
(function(){
...
// DEBUG ADMIN
// ══════════════════════════════════════════════════════
var _vpDbg = window.__VIVA_DEBUG === true;
function vpLog(){
  if(!_vpDbg) return;
  var args = Array.prototype.slice.call(arguments);
  args.unshift('%c[VP]','color:#E8600A;font-weight:bold');
  console.log.apply(console, args);
}
function vpError(){
  if(!_vpDbg) return;
  var args = Array.prototype.slice.call(arguments);
  args.unshift('%c[VP ERROR]','color:#E84545;font-weight:bold');
  console.error.apply(console, args);
}
if(_vpDbg){
  console.log('%c[Viva Pre-EVM] Script IIFE cargado OK','color:#0FBE7C;font-weight:bold');
  window.onerror = function(msg,src,line,col,err){
    console.error('[VP] window.onerror →',msg,'|',src+':'+line+':'+col, err||'');
  };
}
...
// ══════════════════════════════════════════════════════
function vpInit(){
  vpLog('vpInit() ejecutado');
  vpLog('vpCfg →', vpCfg);
  vpLog('#viva-preevm-app existe →', !!document.getElementById('viva-preevm-app'));
  vpLog('Screens encontradas →', document.querySelectorAll('#viva-preevm-app [data-screen]').length);
...
function vpGoScreen(id){
  vpLog('vpGoScreen("'+id+'")');
  var screens = document.querySelectorAll('#viva-preevm-app [data-screen]');
  vpLog('  screens encontradas:', screens.length);
  screens.forEach(function(el){ el.style.display='none'; });
  var el = document.querySelector('#viva-preevm-app [data-screen="'+id+'"]');
  if(el){ el.style.display='block'; vpLog('  screen "'+id+'" visible OK'); }
  else { vpError('  screen "'+id+'" NO ENCONTRADA en el DOM'); }
}
...
// PDF
// ══════════════════════════════════════════════════════
var LOGO_PDF_B64 = window._viva_logo_b64 || '';
if(_vpDbg){ window._viva_preevm_logo_preview_len = LOGO_PDF_B64.length; vpLog('LOGO_PDF_B64 len:',LOGO_PDF_B64.length); }
...
function vpGeneratePDF(){
  ...
  var d=vpResult;
  var W=210,H=297,M=20,TW=170;
  var coR=232,coG=96,coB=10; // naranja #E8600A
  var cy=33; // cursor Y global (compartido por newPage y addSection)

  // ── Degradado simulado con bandas horizontales ────────────────
  var drawGradient=function(x,y0,w,h,r1,g1,b1,r2,g2,b2){
    var bands=24,bh=h/bands;
    for(var i=0;i<bands;i++){
      var t=i/bands;
  ...
  // PORTADA — página 1
  // ════════════════════════════════════════════════════════
  // Fondo degradado naranja → dorado
  drawGradient(0,0,W,H,232,96,10,245,170,28);

  // Elemento decorativo esquina inferior derecha (simula documentos sobre mesa)
  doc.setFillColor(245,148,20);doc.ellipse(178,228,52,52,'F');
  doc.setFillColor(238,130,12);doc.ellipse(183,262,42,42,'F');
  doc.setFillColor(228,112,8); doc.ellipse(166,254,36,36,'F');
  // Simular hoja de papel con líneas
  doc.setFillColor(255,242,210);doc.roundedRect(148,196,58,42,2,2,'F');
  doc.setFillColor(255,255,255);doc.roundedRect(152,200,50,36,1,1,'F');
  doc.setFillColor(205,212,222);
  doc.rect(157,206,30,1.2,'F');doc.rect(157,210,22,1.2,'F');
  ...
  // Barra vertical blanca (acento del título)
  doc.setFillColor(255,255,255);doc.rect(M,66,5,72,'F');
  ...
  // Elemento decorativo circular (evoca foto Sydney / paisaje)
  doc.setFillColor(coR,coG,coB);doc.ellipse(168,255,40,40,'F');
  doc.setFillColor(245,162,30);doc.ellipse(168,255,33,33,'F');
  doc.setFillColor(235,148,18);doc.ellipse(168,255,24,24,'F');
  doc.setFont('helvetica','bold');doc.setFontSize(7.5);doc.setTextColor(255,255,255);
  ...
  // Bloque de identificación del candidato
  doc.setFillColor(250,251,253);doc.roundedRect(M,cy,TW,30,3,3,'F');
  doc.setFillColor(coR,coG,coB);doc.rect(M,cy,3,30,'F');
  doc.setFont('helvetica','bold');doc.setFontSize(14);doc.setTextColor(10,22,42);
  doc.text((d.nom||'')+' '+(d.ape||''),M+8,cy+11);
  ...
  // Sección 4 — Marco regulatorio (ANZSCO)
  var anzscoBody='El sistema migratorio australiano utiliza la clasificación ANZSCO para identificar ocupaciones calificadas. Para validar la experiencia se requiere un Skills Assessment emitido por autoridades competentes (CPA Australia, CA ANZ o IPA).';
  if((d.anzsco||[]).length>0){
    anzscoBody+='\n\nCódigos ANZSCO orientativos (no definitivos en esta fase):\n';
    (d.anzsco||[]).forEach(function(a){anzscoBody+=a.code+' – '+a.name+(a.note?' ('+a.note+')':'')+'\n';});
  ...
  // Secciones 6 y 7 solo para apto / parcial
  if(d.viability!=='no-apto'){
    addSection(6,'Definición de la Evaluación de Viabilidad Migratoria (EVM)',
      'La EVM es un análisis técnico-jurídico profundo realizado por nuestro Agente Migratorio Registrado. En ella se define la estrategia legal exacta, se calcula el puntaje real, se identifican los riesgos normativos (incluyendo el análisis de antecedentes que puedan impactar futuras solicitudes) y se establece una hoja de ruta con tiempos y costos totales del proceso. Solo se recomienda cuando, como en tu caso, existe una base jurídica real para avanzar.');
    addSection(7,'Honorarios y condición comercial',
  ...
  // Bloque disclaimer / validación profesional
  if(cy+38>H-16){newPage();}
  doc.setFillColor(248,249,250);doc.roundedRect(M,cy,TW,38,2,2,'F');
  doc.setFillColor(coR,coG,coB);doc.rect(M,cy,2.5,38,'F');
  cy+=6;
  ...
  if(vpResultUrl){
    if(cy>H-18){newPage();}
    doc.setFont('helvetica','normal');doc.setFontSize(8.5);doc.setTextColor(coR,coG,coB);
    doc.text('Ver informe online: '+vpResultUrl,M,cy);
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function(){
    vpLog('DOMContentLoaded disparado → llamando vpInit()');
    vpInit();
  });
} else {
  // DOM ya listo (script cargó después de DOMContentLoaded)
  vpLog('DOM ya listo → llamando vpInit() directo');
  vpInit();
}
...
VPJS;
    ...
    $js_output = viva_preevm_escape_js_emoji( ob_get_clean() );
